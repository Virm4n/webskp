...
if (! function_exists('sc_popup')) {
	function sc_popup($attr, $content = null)
	{
		...
		if ($button) {
			$output .= '<a href="#'. esc_attr($uid) .'" rel="prettyphoto" data-type="inline" class="popup-link button"><span class="button_label">'. wp_kses($title, mfn_allowed_html()) .'</span></a>';
		} else {
			$output .= '<a href="#'. esc_attr($uid) .'" rel="prettyphoto" data-type="inline" class="popup-link">'. wp_kses($title, mfn_allowed_html()) .'</a>';
		}
